...
import { Card, Form, Input, Button, message, Space, Typography, Alert, List, Spin } from 'antd';
import { FileTextOutlined, DatabaseOutlined, CloudOutlined, ReloadOutlined } from '@ant-design/icons';
...
const LogMethod = ({ network, walletAddress, onRecord }) => {
  ...
  const initializeContract = async () => {
    try {
      ...
      const contractInstance = new ethers.Contract(address, contractABI.abi, provider);
      setContract(contractInstance);
      console.log('合约初始化成功:', address);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <Paragraph style={{ color: '#666', marginBottom: '24px' }}>
        通过调用智能合约函数来写入日志数据到区块链上。
      </Paragraph>

      <Alert
        message="智能合约交互"
        description="调用sayHelloTo(name)函数写入问候数据到区块链。调用合约需要支付gas费。"
        type="info"
        showIcon
      ...
      {/* The Graph数据显示区域 */}
      <Card title="The Graph数据查询" style={{ marginTop: '20px', marginBottom: '20px' }}>
      ...
      <Card title="学习要点" size="small">
        <div style={{ fontSize: '12px', color: '#666' }}>
          ...
          <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
            ...
            <li>handleWriteLog：处理合约函数调用</li>
            <li>testContractConnection：测试合约连接</li>
            <li>fetchSubgraphData：从The Graph获取数据</li>
          </ul>
